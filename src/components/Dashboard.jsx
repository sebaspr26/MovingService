    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {trucks.length > 0 && !loading && (
          <div className="-mr-3 sm:-mr-6 px-5 py-3 bg-gray-900 border border-gray-800 border-r-0 rounded-l-xl text-sm font-medium flex items-center gap-3">
      ...
      <div className="fixed bottom-6 right-6 z-40 sm:hidden flex flex-col items-end gap-2">
        {fabOpen && (
          <div className="flex flex-col gap-2 items-end animate-in fade-in slide-in-from-bottom-2 duration-200">
            {trucksWithCycles.length > 0 && (
              <>
                <button onClick={() => { openQuickAdd('order'); setFabOpen(false) }}
                  className="px-3 py-2 bg-green-600/20 border border-green-600/40 text-green-400 rounded-lg text-xs font-medium hover:bg-green-600/30 transition-colors flex items-center gap-1.5 backdrop-blur-sm shadow-lg">
                ...
                <button onClick={() => { setShowExpenseModal(true); setFabOpen(false) }}
                  className="px-3 py-2 bg-red-600/20 border border-red-600/40 text-red-400 rounded-lg text-xs font-medium hover:bg-red-600/30 transition-colors flex items-center gap-1.5 backdrop-blur-sm shadow-lg">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                  </svg>
                  Gasto
            ...
            <button onClick={() => { openTruckModal(); setFabOpen(false) }}
              className="px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-500 transition-colors shadow-lg flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />
              </svg>
              Camion
        ...
        <button onClick={() => setFabOpen(!fabOpen)}
          className={`p-3 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-500 transition-all duration-200 ${fabOpen ? 'rotate-45' : ''}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
        </button>
