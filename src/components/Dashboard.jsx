...
export default function Dashboard() {
  ...
  const mobileNavCards = [
    ...
    if (item.adminOnly) return isSuperAdmin(session) || session?.user?.user_metadata?.role === 'admin'
  ...
  return (
    <div>
      {/* Mobile welcome home — visible only on mobile */}
      <div className="block lg:hidden mb-2">
        ...
        <div className="flex items-center justify-between mb-3 px-1">
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Camiones</h3>
        </div>
      </div>
