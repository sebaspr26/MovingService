              ...
              {leaseDrivers.map(driver => {
                ...
                return (
                  <div
                    key={driver.id}
                    onClick={() => isActive && setSelectedDriver(driver)}
                    className={`bg-gray-900 border rounded-xl p-4 flex flex-col gap-3 transition-colors ring-1 ring-green-900/30 ${isActive ? 'border-green-900/40 hover:border-green-600/50 cursor-pointer hover:bg-gray-900/80' : 'border-gray-800/40 opacity-60'}`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0 overflow-hidden"
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                      {driver.phone && <div><p className="text-gray-500 mb-0.5">Teléfono</p><p className="text-gray-200">{driver.phone}</p></div>}
                      {driver.email && <div><p className="text-gray-500 mb-0.5">Email</p><p className="text-gray-200 truncate">{driver.email}</p></div>}
                      {driver.license_number && <div><p className="text-gray-500 mb-0.5">CDL #</p><p className="text-gray-200 font-mono">{driver.license_number}{driver.license_state ? ` (${driver.license_state})` : ''}</p></div>}
                    </div>
                    {(driver.license_expiry || driver.medical_card_expiry) && (
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-800">
                        {driver.license_expiry && licBadge && <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${licBadge.bg}`}>CDL {fmtDate(driver.license_expiry)} <span className="opacity-75">({licBadge.label})</span></span>}
                        {driver.medical_card_expiry && medBadge && <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${medBadge.bg}`}>Med {fmtDate(driver.medical_card_expiry)} <span className="opacity-75">({medBadge.label})</span></span>}
                      </div>
                    )}
                    {isActive && (
                      <div className="pt-1 border-t border-gray-800 flex items-center justify-between">
                        <span className="text-[10px] text-gray-600">Click para gestionar pagos</span>
                        <svg className="w-3.5 h-3.5 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
                      </div>
                    )}
                  </div>
                )
