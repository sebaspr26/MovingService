-- Add company_id to orders, drivers, and trailers for multi-company isolation
...
ALTER TABLE trailers ADD COLUMN IF NOT EXISTS company_id uuid REFERENCES company_settings(id) ON DELETE SET NULL;
...
UPDATE trailers SET company_id = (SELECT id FROM company_settings ORDER BY created_at ASC LIMIT 1) WHERE company_id IS NULL;
...
CREATE INDEX IF NOT EXISTS trailers_company_id_idx ON trailers(company_id);
